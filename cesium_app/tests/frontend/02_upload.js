casper.test.begin('upload', function suite(test) {
    casper.start('http://localhost:5000', function() {
        ...
        casper.then(function(){
            ...
            utils.dump('Uploading files from data path: ' + casper.cli.get('data-path'));
            this.page.uploadFile('#headerfile',
                                 casper.cli.get('data-path') + '/asas_training_subset_classes.dat');
            this.page.uploadFile('#zipfile',
                                 casper.cli.get('data-path') + '/asas_training_subset.tar.gz');
