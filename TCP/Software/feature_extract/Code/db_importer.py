...
class vosource_classification_obj:
    ...
    def get_class_xml_string(self):
        ...
        for human_algo in list(self._classes.keys()):
            self.class_xml_string += '    <Classification type="%s">\n' % (human_algo)
            for catalog_tcp in list(self._classes[human_algo].keys()):
                for src_name in list(self._classes[human_algo][catalog_tcp].keys()):
        ...
        <comments></comments>
                    ...
                    for class_schema_name in list(self._classes[human_algo][catalog_tcp][src_name].keys()):
                        #self.class_xml_string += '        <CLASS_SCHEMA type="%s">\n' % (class_schema_name)
                        for class_name in list(self._classes[human_algo][catalog_tcp][src_name][class_schema_name].keys()):
...
class Source:
        ...
        if make_xml_if_given_dict and self.coerced_to_dict:
            ...
            if doplot:
                self.plot_from_dict(self.x_sdict,interactive= \
                                    interactive,bands=list(self.x_sdict['ts'].keys()))
    ...
    def coerce_sdict(self,metakeys=["src_id",'ra_rms',"dec_rms","ra",\
        ...
        all_entries = list(self.sdict.keys())
    ...
    def xml_to_source_dict(self,xml_handle):
        ...
        if 0:
            ...
            band_name_with_data = list(ts_without_combo.keys())[0]
            ...
            t_tup_list.sort()
            i_sorted_list = [a_b[1] for a_b in t_tup_list]
            combo_band_dict['m'] = [combo_band_dict['m'][i] for i in i_sorted_list]
            combo_band_dict['m_err'] = [combo_band_dict['m_err'][i] for i in i_sorted_list]
            combo_band_dict['t'] = [combo_band_dict['t'][i] for i in i_sorted_list]

            t_tup_list = []
            for i,t_val in enumerate(combo_band_dict['limitmags']['t']):
                t_tup_list.append((t_val,i))
            t_tup_list.sort()
            i_sorted_list = [a_b1[1] for a_b1 in t_tup_list]
            combo_band_dict['limitmags']['lmt_mg'] = [combo_band_dict['limitmags']['lmt_mg'][i] for i in i_sorted_list]
            combo_band_dict['limitmags']['t'] = [combo_band_dict['limitmags']['t'][i] for i in i_sorted_list]
    ...
    # obsolete:
    def xml_to_source_dict__old_20090612_1day(self,xml_handle):
        """
        takes the XML string and makes a dict
        """
        #if not use_amara:
        ...
        for classfn_branch in classification_branches:
            ...
            try:
                for source_branch in classfn_branch.findall('source'):
                    if (source_branch.get('type') == 'tutor-data-ingest') or \
                       (source_branch.get('type') == 'tcp'):
                        correct_branch = True
                        break
            except:
                pass
    ...
    def simpletimeseriesxml_to_source_dict(self, xml_handle):
        """ This reads the given XML, and translates into a db_importer.Source style source dict (sdict).
        """
        xml_ns = "http://lyra.berkeley.edu/~pteluser/simpletimeseries" # This is stored in the simpletimeseries xml.
        #  TODO: I suppose I could try to parse this from a given simpletimeseries xml string & then use it.
        root = "{%s}SimpleTimeseries" % (xml_ns)
        prefix = "{%s}" % (xml_ns)
        ...
        self.elemtree = ElementTree.fromstring(xml_str)
        xmld_data = xmldict.ConvertXmlToDict(self.elemtree)
        # xmldict form:
        #  data = vosource_parse.vosource_parser(_xmlfpath).d  = xmldict.ConvertXmlToDict(self.elemtree)
        #data.data['VOSOURCE']['Features']['feature'][0].keys()
        #['origin', 'name', 'err', 'filter', 'val', 'description']
        srcid_uri = xmld_data[root].get(prefix+'ID','')
        if 'ivo' in srcid_uri:
            if srcid_uri.count('#') == 0:
                srcid_num = int(srcid_uri[srcid_uri.rfind(' ')+1:])
            else:
        ...
        try:
            ...
            self.x_sdict.update({"src_id": 'UNKNOWN'})
        try:
            self.x_sdict.update({"ra": float(xmld_data[root].get(prefix+'RA',0))})
        except:
            self.x_sdict.update({"ra": 0})
        try:
            self.x_sdict.update({"dec": float(xmld_data[root].get(prefix+'DEC',0))})
        except:
            self.x_sdict.update({"dec": 0})
        try:
            self.x_sdict.update({"ra_rms": float(xmld_data[root].get(prefix+'RA_RMS',0))})
        except:
            self.x_sdict.update({"ra_rms": 0})
        try:
            self.x_sdict.update({"dec_rms": float(xmld_data[root].get(prefix+'DEC_RMS',0))})
        except:
            self.x_sdict.update({"dec_rms": 0})

        filt_ids = []
        filt_fields = self.elemtree.findall(prefix+'FIELD')
        for filt_field_dict in filt_fields:
            if "fld" in list(filt_field_dict.keys()):
                filt_ids.append(filt_field_dict.get("fld"))

        data_dict = {}
        for filt_id in filt_ids:
            data_dict[filt_id] = {'m':[], 'm_err':[], 't':[], 'limitmags':{'t':[], 'lmt_mg':[]}} # 'ordered_column_names': array_names, 'ucds': ucds, 'IDs': IDs, 'units': units,
        for ts_elem in xmld_data[root].get(prefix+'SERIES',{}).get(prefix+'ELEM',{}):
            t = float(ts_elem.get(prefix+'TIME',{}).get(prefix+'T',0))
            for k in list(ts_elem.keys()):
                if 'MAG' in k:
                    filt_id = ts_elem[k].get('fld',"")
                    if 'limit' in list(ts_elem[k].keys()):
                        data_dict[filt_id]['limitmags']['t'].append(t)
                        mag = float(ts_elem[k].get(prefix+'VAL',0))
                        data_dict[filt_id]['limitmags']['lmt_mg'].append(mag)
                    else:
                        data_dict[filt_id]['t'].append(t)
                        mag = float(ts_elem[k].get(prefix+'VAL',0))
                        err = float(ts_elem[k].get(prefix+'ERR',0))
                        data_dict[filt_id]['m'].append(mag)
                        data_dict[filt_id]['m_err'].append(err)

        self.x_sdict['ts'] = data_dict

        if 0:
            # # # 20100521: dstarr disables this due to complications with watt_per_m2_flux and just the general sloppiness of combining filters and their magnigueds.  Ideally there would be some soft of offset magnitude used for each band (like the watt_per_m2_flux, and specific features would act only on this internal combined flux(t) dataset
            # TODO: generate a combo band, including limit_mags... see L ow618
            ###############################
            # NOTE: (20100215) this is derived from xml_to_source_dict()
            # I'm generating a combined-band which due to a combined number of data points, should
            #    be used for band-invariant features.  This is useful for PTF since data is sparse and
            #    spread bi-monthly over 2 bands.  It is a kludge because astrophysical objects have
            #    different brightnesses in different bands.  Hopefully features which dominate
            #    effective classification prefer more samples than magnitude accuracy/offsets.
            ts_without_combo = {}
            for k,v in self.x_sdict['ts'].items():
                if k != 'combo_band':
                    ts_without_combo[k] = v
            ...
            for i,t_val in enumerate(combo_band_dict.get('t',[])):
                t_tup_list.append((t_val,i))
            t_tup_list.sort()
            i_sorted_list = [a_b2[1] for a_b2 in t_tup_list]
            combo_band_dict['m'] = [combo_band_dict['m'][i] for i in i_sorted_list]
            combo_band_dict['m_err'] = [combo_band_dict['m_err'][i] for i in i_sorted_list]
            combo_band_dict['t'] = [combo_band_dict['t'][i] for i in i_sorted_list]
            ...
            for i,t_val in enumerate(combo_band_dict['limitmags']['t']):
                t_tup_list.append((t_val,i))
            t_tup_list.sort()
            i_sorted_list = [a_b3[1] for a_b3 in t_tup_list]
            combo_band_dict['limitmags']['lmt_mg'] = [combo_band_dict['limitmags']['lmt_mg'][i] for i in i_sorted_list]
            combo_band_dict['limitmags']['t'] = [combo_band_dict['limitmags']['t'][i] for i in i_sorted_list]

            self.x_sdict['ts']['combo_band'] = combo_band_dict
        ###############################
        self.coerced_to_dict = True
    ...
    def source_dict_to_xml__simptimeseries(self, sdict):
        ...
        """
        xml_ns = "http://lyra.berkeley.edu/~pteluser/simpletimeseries"
        SimpleTimeseries = ElementTree.Element("SimpleTimeseries")
        SimpleTimeseries.set('xmlns:xsi', "http://www.w3.org/2001/XMLSchema-instance")
        SimpleTimeseries.set('xsi:schemaLocation', "http://lyra.berkeley.edu/~pteluser/simpletimeseries http://lyra.berkeley.edu/~pteluser/simpletimeseries.xsd")
        SimpleTimeseries.set('xmlns', xml_ns) # This is required for web-browser rendering using xsd, css, etc.   Also note that with this, we have to then prefix the SimpleTimeseries <tag> with it when using xml_dict.py, etc...
        DESCRIPTION = ElementTree.SubElement(SimpleTimeseries, "DESCRIPTION")
        DESCRIPTION.text = "\nThis is using John Brewer's SimpleTimeseries format, referencing example simple_lightcurve_sn1991bg.xml\n"
        ElementTree.SubElement(SimpleTimeseries, "ID").text =  str(sdict.get("src_id",""))
        ElementTree.SubElement(SimpleTimeseries, "RA").text =  str(sdict.get("ra",""))
        ElementTree.SubElement(SimpleTimeseries, "DEC").text =  str(sdict.get("dec",""))
        ElementTree.SubElement(SimpleTimeseries, "RA_RMS").text =  str(sdict.get("ra_rms",""))
        ElementTree.SubElement(SimpleTimeseries, "DEC_RMS").text =  str(sdict.get("dec_rms",""))

        TIMESYS = ElementTree.SubElement(SimpleTimeseries, "TIMESYS")
        ElementTree.SubElement(TIMESYS, "TimeType", ucd="time;pos.frame;pos.heliocentric", unit="day").text = "hjd"
        ElementTree.SubElement(TIMESYS, "TimeZero", ucd="time.epoch;arith.zp", unit="day").text = "0"
        TIMESYS.append(ElementTree.Comment("describe the progress of the time axis"))
        ...
        if "ts" in sdict:
            filts = list(sdict['ts'].keys())
            if 'combo_band' in filts:
                filts.remove('combo_band')

            for filt_name in filts:
                ElementTree.SubElement(SimpleTimeseries, "BAND", ucd="instr.filter;em.opt", bandid=filt_name, description="This is band: " + filt_name).text = filt_name
            for filt_name in filts:
                ElementTree.SubElement(SimpleTimeseries, "FIELD", fld=filt_name, bandid=filt_name, ucd="opt;phot;" + filt_name, datatype="float", unit="mag").text = filt_name + "-band photometry"
                #ElementTree.SubElement(SimpleTimeseries, "FIELD", fld=filt_name+"_limit", bandid=filt_name, ucd="opt;phot;" + filt_name, datatype="float", unit="mag").text = filt_name + "-band Upper Limit"

            tup_list = [] # [(<time sortable elements>, <dict which will be update() in time condensed dictionary>), ...]
            for filt_id in filts:
                if len(sdict['ts'][filt_id].get('limitmags',{}).get('lmt_mg',[])) > 0:
                    for i,t in enumerate(sdict['ts'][filt_id]['limitmags']['t']):
                        tup_list.append((t, filt_id, True, {(filt_id, True): \
                                  {'mag':sdict['ts'][filt_id]['limitmags']['lmt_mg'][i]}}))
                for i,t in enumerate(sdict['ts'][filt_id]['t']):
                    tup_list.append((t, filt_id, False, {(filt_id, False): \
                              {'mag':sdict['ts'][filt_id]['m'][i],
                               'm_err':sdict['ts'][filt_id]['m_err'][i]}}))
            tup_list.sort()
            time_condensed_dict = {}
            t_prev = 0
            t_list_sorted = []
            for (t, filt_id, is_limit, datadict) in tup_list:
                # TODO: could do some float comparison, within expected float precision:
                if t > (t_prev + 0.0001):
                    time_condensed_dict[t] = datadict
                    t_prev = t
                    t_list_sorted.append(t)
                else:
                    if t != t_prev:
                        t = t_prev # since this is within a float precision range
                    time_condensed_dict[t].update(datadict)

            SERIES = ElementTree.SubElement(SimpleTimeseries, "SERIES")

            for i,t in enumerate(t_list_sorted):
                ELEM = ElementTree.SubElement(SERIES, "ELEM", row=str(i))
                TIME = ElementTree.SubElement(ELEM, "TIME")
                ElementTree.SubElement(TIME, "T").text=str(t)
                for filt_tup,mag_dict in time_condensed_dict[t].items():
                    (filt_id, is_limit) = filt_tup
                    if is_limit:
                        #filt_id += "_limit"
                        MAG = ElementTree.SubElement(ELEM, "MAG", fld=filt_id, limit="upper")
                        ElementTree.SubElement(MAG, "VAL").text=str(mag_dict['mag'])
                        #filt_id += "_limit"
                        #MAG = ElementTree.SubElement(ELEM, "MAG", fld=filt_id)
                        #ElementTree.SubElement(MAG, "VAL").text=str(mag_dict['mag'])
                    else:
                        MAG = ElementTree.SubElement(ELEM, "MAG", fld=filt_id)
                        ElementTree.SubElement(MAG, "VAL").text=str(mag_dict['mag'])
                        ElementTree.SubElement(MAG, "ERR").text=str(mag_dict['m_err'])

        #######################
        import io
        strio = io.StringIO()
        add_pretty_indents_to_elemtree(SimpleTimeseries, 0)
        tree = ElementTree.ElementTree(SimpleTimeseries)
        tree.write(strio, encoding="UTF-8")

        full_str = strio.getvalue()
        i_simptime = full_str.find('<SimpleTimeseries')
        out_str = full_str[:i_simptime] + \
                  '<?xml-stylesheet type="text/xsl" href="http://lyra.berkeley.edu/~pteluser/timeseries_xhtml.xsl"?>\n' + \
                  full_str[i_simptime:]
        return out_str
...
class PositionExtractor:
    ...
    def setup_remote_connection(self,host="localhost",port=8000):
        ...
        self.server = xmlrpc.client.ServerProxy("http://%s:%i" % (host, port))
...
def dotest1():
    ...
    pe.search_pos()
    # this will write an xml source file for each source
    #
    # take a look at the pe.sources list.
    pe.summary_plot()
    pe.ds9_summary()

    # to get the data from a given band
    interesting_source = pe.sources[0]
    filt = "g"
    global t, m, m_err
    (t,m,m_err) = (interesting_source.d['ts'][filt]['t'], interesting_source.d['ts'][filt]['m'], interesting_source.d['ts'][filt]['m_err'])
