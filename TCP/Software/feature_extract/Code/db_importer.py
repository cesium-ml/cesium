	...
	def xml_to_source_dict__old_20090612_1day(self,xml_handle):
	    ...
	    try:
            ...
            for filt_field_dict in filt_fields:
                if "fld" in filt_field_dict.keys():
            ...
            for ts_elem in xmld_data[root].get(prefix+'SERIES',{}).get(prefix+'ELEM',{}):
                ...
                for k in ts_elem.keys():
                    if 'MAG' in k:
                        filt_id = ts_elem[k].get('fld',"")
                        if 'limit' in ts_elem[k].keys():
	        ...
	        #    effective classification prefer more samples than magnitude accuracy/offsets.
                ts_without_combo = {}
                for k,v in self.x_sdict['ts'].iteritems():
                    if k != 'combo_band':
                        ts_without_combo[k] = v
			...
			filts = sdict['ts'].keys()
					...
					t = t_prev # since this is within a float precision range
                        ...
                        for i,t in enumerate(t_list_sorted):
                            ...
                            for filt_tup,mag_dict in time_condensed_dict[t].iteritems():
	...
	def source_dict_to_xml(self,sdict):
		...
		if sdict.has_key("features"):
			...
			for filt_name,filt_dict in sdict["features"].iteritems():
				for feat_name,feat_str_val in filt_dict.iteritems():
					sss += "%s<Feature>\n" % (__tab__*xmllevel)
					xmllevel += 1
...
"""
		...
		for s in self.sources:
			...
			src_id = s.d['src_id']
			vv = s.d['ts'].values()
			ss = [band + "=" + str(len(val['t'])) for band, val in s.d['ts'].iteritems()]
