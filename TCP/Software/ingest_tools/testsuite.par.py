parameters = {
    ...
    'test_suite__use_tunnel_mysql_server':True, # obsolete?
    'test_suite__test_all_functionality':False, #False: do system install & quick sanity checks.  True: test astrometry/pairitel, etc...
    'test_suite__force_astrom_photometry':False,#True: Test mos.fits reductions
    'xmlrpc_server_name':"127.0.0.1", #SVN: 127.0.0.1 # Must be non-local IP if XMLRPC Server is to be called by other computers (eg.: 192.168.1.45).
    ...
    'remote_hostname':'192.168.1.55', #obsolete? # Only usable within Evans 260 LAN.
    ...
    'ingest_tools_pars':{ \
        ...
        'rdb_features_db_name':'source_test_db', #USE SOURCE 'features_test_db',
        ...
        'sdss_astrom_repo_host_ip':"localhost",
        'sdss_astrom_repo_dirpath':'$HOME/scratch/TCP_tests/sdss_astrom_repository',
    },
    'testvals_object_database_table_columns':{ \
        'pairitel_events_a':12,
        'pairitel_ingest_accounting':6,
        'obj_srcid_lookup':3,
        'footprint_values':4,
        'footprint_regions':5,
    },
    ...
    'testvals_ptel_feature_values':[ \
      (5, {'old_dc':14.40365, 'wei_av_uncertainty':240.014042127, \
           'median':14.40975, \
           'weighted_average':14.4256568054,'first_freq':0.0307628304132,\
           'std':0.254517518498})],
...
CREATE TABLE lbl_ingest_acct (range_id INT, id_low BIGINT, id_high BIGINT, id_high_initial BIGINT, dtime_touch DATETIME, PRIMARY KEY (range_id));

##########
# PTF Object tables:
CREATE TABLE ptf_events (footprint_id INT UNSIGNED, %s bogus FLOAT, suspect FLOAT, unclear FLOAT, maybe FLOAT, realish FLOAT, realbogus FLOAT,
                         PRIMARY KEY (id),
                         INDEX(footprint_id));
...
echo "
USE %s;
ALTER TABLE %s.ptf_events CHANGE htmID htmID25 BIGINT;
...
CREATE TABLE pairitel_ingest_accounting (proj VARCHAR(8), obj SMALLINT UNSIGNED, obs SMALLINT UNSIGNED, filt TINYINT UNSIGNED, obs_dtime DATETIME, ingest_dtime DATETIME, INDEX(obs_dtime), INDEX(proj, obj, obs));
CREATE TABLE pairitel_events_a (obj_id INT UNSIGNED AUTO_INCREMENT, footprint_id INT UNSIGNED, filt TINYINT UNSIGNED, t DOUBLE, jsb_mag FLOAT, jsb_mag_err FLOAT, ra DOUBLE, decl DOUBLE, ra_rms FLOAT, dec_rms FLOAT, PRIMARY KEY (obj_id), INDEX(footprint_id));
...
CREATE TABLE srcid_lookup (src_id INT UNSIGNED AUTO_INCREMENT, ra DOUBLE, decl DOUBLE, ra_rms FLOAT, dec_rms FLOAT, nobjs SMALLINT UNSIGNED, feat_gen_date DATETIME, feats_used_filt VARCHAR(20), PRIMARY KEY (src_id), INDEX(feat_gen_date));
...
if parameters['test_suite__use_tunnel_mysql_server'] == True:
    parameters['mysql_hostname'] = parameters['local_hostname']
    parameters['mysql_username'] = parameters['local_username']
    parameters['mysql_port'] = parameters['local_mysql_port']
    parameters['ssh_port'] = parameters['local_ssh_port']
    parameters['TCP_path'] = os.path.expandvars(parameters['local_TCP_path'])
else:
    parameters['mysql_hostname'] = parameters['remote_hostname']
    parameters['mysql_username'] = parameters['remote_username']
    parameters['mysql_port'] = parameters['remote_mysql_port']
    parameters['ssh_port'] = parameters['remote_ssh_port']
    parameters['TCP_path'] = os.path.expandvars(parameters['remote_TCP_path'])
parameters['ingest_tools_pars']['sdss_astrom_repo_user'] = parameters['mysql_username']
parameters['ingest_tools_pars']['rdb_host_ip_2'] = parameters['mysql_hostname']
parameters['ingest_tools_pars']['rdb_host_ip_3'] = parameters['mysql_hostname']
parameters['ingest_tools_pars']['rdb_host_ip_4'] = parameters['mysql_hostname']
parameters['ingest_tools_pars']['rdb_features_host_ip'] = parameters['mysql_hostname']
parameters['ingest_tools_pars']['source_region_lock_host_ip'] = parameters['mysql_hostname']
parameters['ingest_tools_pars']['footprint_host_ip'] = parameters['mysql_hostname']
parameters['ingest_tools_pars']['rdb_user'] = parameters['mysql_username']
parameters['ingest_tools_pars']['source_region_lock_user'] = parameters['mysql_username']
...
parameters['ingest_tools_pars']['ptf_sub_table_columns_list'] = ingest_tools.pars['ptf_sub_table_columns_list']
