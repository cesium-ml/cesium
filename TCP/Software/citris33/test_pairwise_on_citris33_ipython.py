...
def update_combo_results(combo_results_dict={}, ipython_return_dict={}):
    ...
    """
    for class_name in list(ipython_return_dict.keys()):
        if class_name not in combo_results_dict:
            combo_results_dict[class_name] = {}
        for r_clfr_name in list(ipython_return_dict[class_name].keys()):
            if r_clfr_name not in combo_results_dict[class_name]:
                combo_results_dict[class_name][r_clfr_name] = {}
            for i_set in list(ipython_return_dict[class_name][r_clfr_name].keys()):
...
class Resample_Pairwise_Classify:
    ...
    def load_pairwise_classifier(self, fpath='', pair_path_replace_tup=None):
        ...
        """
        fp=open(fpath,'rb')
        self.pairwise_classifier_dict=pickle.load(fp)
        fp.close()
    ...
    def organize_sciclassdict_by_percent_niters(self, arff_sciclass_dict):
        ...
        for class_name, class_dict in arff_sciclass_dict.items():
    ...
    def ipython_task(self, src_id='', percent='', niters='', store_confids=True,
     ...
     -> returns: classified results in a dict?  like an ipython task of:
        ...
        if use_r_classifier:
            ...
            print()
            return_dict = {}
            for perc, perc_dict in perc_niter_sciclassdict.items():
                if perc not in return_dict:
                    return_dict[perc] = {}
                for niter, sciclassdict in perc_dict.items():
...
def non_ipython_main(pars, pwiseclass_id_pklfpath={}):
    ...
    pars['niters'] = list(range(int(pars['niters'])))
    ###
    ...
    for pw_classifier_name, pw_classifier_dict in pwiseclass_id_pklfpath.items():
        ...
        for perc, perc_dict in perc_niter_classif_dict.items():
            for niter, classif_dict in perc_dict.items():
...
def ipython_main(pars, pwiseclass_id_pklfpath={}, apply_classifier_to_all_srcids=True):
    ...
    for pw_classifier_name, pw_classifier_dict in pwiseclass_id_pklfpath.items():
        ...
        if (len(pars['crossvalid_pairwise_classif_dirpath']) == 0) or (apply_classifier_to_all_srcids):
            ...
            pairwise_trainset = pickle.load(gzip.open(pairwise_trainset_fpath))
            classifier_srcid_list = []
            for class_name, class_dict in pairwise_trainset.items():
...
try:
    ...
    print "EXCEPT: rcp.load_pairwise_classifier(%s, %s)"
        ...
        for perc in pars['percent']:
            for set_num in range(int(pars['niters'])):
    ...
    for pw_classifier_name, all_classif_dict in pw_all_classif_dict.items():
        for perc, perc_dict in all_classif_dict.items():
            for niter, classif_dict in perc_dict.items():
                WekaPairwiseClassification.tally_correctly_classified_for_percent(perc, niter,
                                                                                  pw_classifier_name,
                                                classif_dict,
    ...
    ###### For debugging:
    import pprint
    for class_name, class_dict in percent_tally_summary_dict.items():
        for pw_class, pw_dict in class_dict.items():
        ...
        for classifier_dict in r_classifiers:
            for set_num in iter_list:
                ...
                for perc in percent_list:
                    ...
                    if len(list(percent_tally_summary_dict.keys())) == 0:
...
RTaskControllers = test_pairwise_on_citris33_ipython.R_Task_Controllers()"""
    ...
    #import pdb; pdb.set_trace()
    task_id_list = []
    for classifier_name, classifier_dict in classifier_info_dicts.items():
...
if __name__ == '__main__':
    ...
    if 0:
        ...
        combo_results_dict = {}
        perc_plot_result = pickle.load(gzip.open('/global/home/users/dstarr/scratch/perc_plot_results__20110105_0to20at1_7set.pkl.gz'))
        update_combo_results(combo_results_dict=combo_results_dict,
                                 ipython_return_dict=perc_plot_result)

        perc_plot_result = pickle.load(gzip.open('/global/home/users/dstarr/scratch/perc_plot_results__20110105_20to60at1_7set.pkl.gz'))
        update_combo_results(combo_results_dict=combo_results_dict,
                                 ipython_return_dict=perc_plot_result)


        perc_plot_result = pickle.load(gzip.open('/global/home/users/dstarr/scratch/perc_plot_results__20110105_60to100at1_7set.pkl.gz'))
        update_combo_results(combo_results_dict=combo_results_dict,
                                 ipython_return_dict=perc_plot_result)
    ...
    pickle.dump(combo_results_dict, fp, 1)
    fp.close()
