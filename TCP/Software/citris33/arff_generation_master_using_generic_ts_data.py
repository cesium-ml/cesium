...
def generate_xml_str_using_ben_dat(dat_fpath):
...
def get_dat_arffstrs(dat_fpaths=[], percent_list=[], niters=1, include_header=True,
    ...
    for dat_fpath in dat_fpaths:
        ...
        # TODO: here I form the xml string from dat file ala format_csv_getfeats.py

        new_xml_str = generate_xml_str_using_ben_dat(dat_fpath)

        #xml_str = open(xml_fpath).read()
        #new_xml_str = ParseNomadColorsList.get_colors_for_srcid(xml_str=xml_str, srcid=tutor_src_id)
        ...
        for perc in percent_list:
            ...
            for i in niters:
                if write_multiinfo_srcids:
                    ...
                    new_srcid = '%s' % (dat_fpath[dat_fpath.rfind("_") + 1:dat_fpath.rfind(".")])
...
class Arff_Generation_Engine_Tasks:
    ...
    def task_generate_feature_arff_lines(self, pars, dat_fpaths=[], include_header=True,
        ...
        arff_str = get_dat_arffstrs(dat_fpaths, sub_perc_list, sub_iter_list,
...
import matplotlib
matplotlib.use('agg')
sys.path.append(os.path.abspath('/home/pteluser/src/TCP/Software/ingest_tools'))
sys.path.append(os.path.abspath('/home/pteluser/src/TCP/Software/citris33'))
import arff_generation_master_using_generic_ts_data
ArffEngineTasks = arff_generation_master_using_generic_ts_data.Arff_Generation_Engine_Tasks()
"""
    ...
    ParseNomadColorsList = Parse_Nomad_Colors_List(fpath='/home/pteluser/src/TCP/Data/best_nomad_src_list')
    ArffEngineTasks = Arff_Generation_Engine_Tasks()
    out_dict = ArffEngineTasks.task_generate_feature_arff_lines(pars, dat_fpaths=dat_fpaths[:1], include_header=True, write_multiinfo_srcids=write_multiinfo_srcids, ParseNomadColorsList=ParseNomadColorsList)
    ### KLUDGE leave out the last row since it will be reprocessed below:
    result_arff_list.extend(out_dict['arff_rows'][:-1])


    n_src_per_task = 10 # NOTE: is generating PSD(freq) plots within lightcurve.py, should use n_src_per_task = 1, and all tasks should finish.# for ALL_TUTOR, =1 ipcontroller uses 99% memory, so maybe =3? (NOTE: cant do =10 since some TUTOR sources fail)

    imin_list = list(range(0, len(dat_fpaths), n_src_per_task))

    for i_min in imin_list:
        dat_sublist = dat_fpaths[i_min: i_min + n_src_per_task]
        #print
        ##### FOR DEBUGGING:
        #####    - NOTE: will use up memory if run for ~100 iterations
        #ArffEngineTasks = Arff_Generation_Engine_Tasks()
        #out_dict = ArffEngineTasks.task_generate_feature_arff_lines(pars, dat_fpaths=dat_sublist, include_header=False, write_multiinfo_srcids=write_multiinfo_srcids)
        #import pdb; pdb.set_trace()
        #print
        #####
...
out_dict = ArffEngineTasks.task_generate_feature_arff_lines(pars, dat_fpaths=dat_fpaths, include_header=include_header, write_multiinfo_srcids=write_multiinfo_srcids)
sys.stdout.close()
sys.stdout = tmp_stdout
        ...
        ###for debugging:
        #for a,b in inspect.getmembers(tc.get_task_result(taskid, block=False)): print a,b
        #import pdb; pdb.set_trace()
        #print
    #import pdb; pdb.set_trace()
    ####
...
if __name__ == '__main__':
    ...
    if 0:
        ...
        if os.path.exists(xmls_dict_pkl_fpath):
            source_xml_dict = pickle.load(open(xmls_dict_pkl_fpath))
        else:
            #import pdb; pdb.set_trace()
            #print
            ...
            pickle.dump(source_xml_dict,fp,1) # ,1) means a binary pkl is used.
            fp.close()
        ...
        if 0:
            ### For testing:
            xml_str = generate_xml_str_using_ben_dat("/home/pteluser/Dropbox/LCS/LCnew_417.dat")
            import pdb; pdb.set_trace()
            print()

        glob_str = '/home/pteluser/Dropbox/LCS/LCnew_*.dat'
