...
def qso_engine(time,data,error,ltau=3.,lvar=-1.7,sys_err=0.,return_model=False):
    ...
    if ((scipy_version == '0.8.0b1') or (scipy_version == '0.8.0')):
        b1 = (wt*dat).reshape((1,ln))
        b2 = transpose(b1)
        (Tpc,z) = solveh_banded(Tp,b2)
        z =transpose(z)
        z = z[0,:]
        #(Tpc,z) = solveh_banded(Tp,b1); z = z[0,:]
        c1 = wt.reshape((1,ln))
        c2 = transpose(c1)
        (Tpc,z0) = solveh_banded(Tp,c2)
        z0 = transpose(z0)
        z0 = z0[0,:]
    if ((scipy_version == '0.7.0rc2')):
        # This works for scipy __version__='0.7.0rc2' and Nat's use of v1.0
        b1 = (wt*dat).reshape((1,ln))
        (Tpc,z) = solveh_banded(Tp,b1)
        z = z[0,:]
        #(Tpc,z) = solveh_banded(Tp,b1); z = z[0,:]
        c1 = wt.reshape((1,ln))
        (Tpc,z0) = solveh_banded(Tp,c1)
        z0 = z0[0,:]
    else:
...
def qso_fit(time,data,error,filter='r',sys_err=0.0,return_model=False):
    ...
    out_dict['log_chi2_qsonu'] = log(out_dict['chi2_qso/nu'])
    out_dict['log_chi2nuNULL_chi2nu'] = log(out_dict['chi2_qso/nu_NULL'] / out_dict['chi2_qso/nu'])
    ###
