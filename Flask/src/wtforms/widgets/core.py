    ...
    def __call__(self, field, **kwargs):
        ...
        for subfield in field:
            if subfield.type == 'HiddenField':
                ...
                html.append('<tr><th>%s</th><td>%s%s</td></tr>' % (str(subfield.label), hidden, str(subfield)))
                hidden = ''
        if self.with_table_tag:
            html.append('</table>')
        if hidden:
...
class TextArea(object):
    ...
    def __call__(self, field, **kwargs): 
        kwargs.setdefault('id', field.id)
        return HTMLString('<textarea %s>%s</textarea>' % (html_params(name=field.name, **kwargs), escape(str(field._value()))))
...
class Select(object):
    ...
    def render_option(cls, value, label, selected, **kwargs):
        ...
        return HTMLString('<option %s>%s</option>' % (html_params(**options), escape(str(label))))
