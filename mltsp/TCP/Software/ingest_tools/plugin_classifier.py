...
class PluginClassifier(object):
    ...
    def do_classification(self, vosource_list, class_schema_definition_dicts, do_logging=False):
        ...
        class_probs_dict = {}
        for src_id,a_dict in plugin_classification_dict.items():
            #prob_list = []
            class_probs_dict[src_id] = []
            for plugin_name,plugin_dict in a_dict.items():
                prob_list = []
                for class_name,class_dict in plugin_dict.get('probabilities',{}).items():
