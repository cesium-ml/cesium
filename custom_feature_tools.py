...
class MissingRequiredParameterError(Exception):
    '''Exception that is raised when a required parameter is not 
    provided in a function call.
    '''
...
class MissingRequiredReturnKeyError(Exception):
    '''Exception raised when a function reports that it returns a 
    certain parameter but does not in fact return it.
    '''
    def __init__(self,value):
        self.value = value
    def __str__(self):
...
class myFeature(object):
    ...
    def __init__(self,requires,provides):
        """
        'requires' must be a list of variable names required by the 
        function, 'provides' must be a list of the key names of the 
        returned dictionary - the features calculated by a particular 
        function. 'requires' and 'provides' are set as object 
        attributes.
        """
